...
private _secWeapon = if (!isNil{_loadout#1#0}) then {_loadout#1#0} else {""};
private _weaponSystemSpecific = switch (true) do {
    case (_primaryWeapon isKindof ['rhs_weap_mk18_m320', configFile >> 'CfgWeapons']
            ...
            || _secWeapon isKindOf ["rhs_weap_M320", configFile >> 'CfgWeapons']): {GET_CONTAINER_KEYS(arsenal_weap_ugl);};
